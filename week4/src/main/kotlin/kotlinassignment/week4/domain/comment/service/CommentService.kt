...
class CommentService(
    ...
    fun updateComment(toDoCardId: Long, commentId: Long, request: CommentUpdateRequest): CommentResponse {
        ...
        val targetComment = getCommentIfPasswordMatches(commentId, request.password)
    ...
    private fun getCommentIfPasswordMatches(commentId: Long, passwordRequested: String): Comment { // readOnly = true, propagation = PROPAGATION.REQUIRED
        // 암호화 될 password는 서버까지 끌고 오지 않고, DB 서버 내에서 비교 후 일치하면 처리되도록 할 것 
        return commentRepository.findByIdAndPassword(commentId, passwordRequested)
            ?: TODO("새로운 에러 클래스 만들어서 던지게 하기 - Comment 비밀번호가 일치하지 않는 경우(DB의 record의 password와 request의 password)")
    }
}
