...
@ContextConfiguration(classes = [WebTestConfigurationFail3::class]) // @Import(WebTestConfiguration::class)
@WebMvcTest(controllers = [ToDoCardController::class])
class ControllerTestWithFieldInjectionEx2Fail3 : AnnotationSpec() {

    override fun extensions(): List<Extension> = listOf(SpringExtension)
    // 여기를 추가해도 실패함 - WebTestConfiguration에서 TestPropertySource를 못 가져오는 문제
    // - 이런 방식으로는 TestPropertySouce를 가져올 수 없는 듯하다.
    //   - 그나마 가능한 방법으로는 https://stackoverflow.com/questions/57190615/using-one-testpropertysource-for-multiple-test-classes 참고
    //   - 다른 클래스에 TestPropertySouce를 두고 테스트 클래스가 그 클래스를 상속하게 하든지, 그냥 @ActiveProfiles를 쓰든지 
